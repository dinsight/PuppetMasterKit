...
namespace PuppetMasterKit.Graphics.Geometry
{
  public class Segment
  {
    ...
    /// </summary>
    /// <returns><c>true</c>, if colinear was ared, <c>false</c> otherwise.</returns>
    ...
    {
      return Math.Abs((a.X * (b.Y - c.Y) + b.X 
                       * (c.Y - a.Y) + c.X 
                       * (a.Y - b.Y)) / 2) <= Float.EPSILON;
    }
    ...
    public HashSet<Point> Intersect(Segment withSegment)
    {
      ...
      if (withSegment.Slope == null) { //vertical edge
