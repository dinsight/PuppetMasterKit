...
namespace PuppetMasterKit.AI.Goals
{
    ...
    {
        private Func<List<Entity>> interactionEntities;
        ...
        /// </summary>
        /// <param name="interactionEntities">Interaction entities.</param>
        /// <param name="interactionRadius">interactionEntities: entities for 
        /// which activities will be performed</param>
        public GoalToPerformInteraction(Func<List<Entity>> interactionEntities, 
		...
		{
            if (interactionEntities().Count == 0 || agent.Position == null)
                return Vector.Zero;

            if (CheckOngoingActivity())
                return Vector.Zero;

            var currentEntity = interactionEntities()[currentIndex];
            ...
            if (target!=null){
                ...
                if( dist.Magnitude() <= interactionRadius) {
                    if( currentIndex == interactionEntities().Count - 1 || 
                        currentIndex == 0) {
                        direction *= -1;
                    }
